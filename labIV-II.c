...
/**
*this f(x) fills an x and y array with the x,y coords of a polygon from a file which is passed by argv[1] as a char array
*this f(x) returns the real length of the array
*/
int get_corners(char file[], double x[], double y[], int length){
	int i;
...
/**
* this f(x) takes in a file from the command line to be used to calculate the area of a polygon, 
...
int main(int argc, char* arcv[]){
	...
	// /public/csci112sp19/lab4/x_y_polygon.txt <- use this file
	
	/**
	* this if statement is used to prevent errors
    * If an argument is not entered on the command line an error would occur because argv would be large enough to pass to get_corners()
	*/
	if(argc != 1){ 
		printf("%d: Error - Please enter proper command line args\n", argc);
		return 0;
	}else{
		get_corners(arcv[1], x, y, size); //f(x) call
		return 0;
	}
}