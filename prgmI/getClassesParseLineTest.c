...
void sort(FILE *out, classes_t cls[], char* type, int length){
	...
	if((strcmp(type, dayMWF) == 0) || (strcmp(type, dayTR) == 0)){
		...
		/*do{
			if(userTime[0] >= 0 && userTime[0] <= 2){ //if between 0000 and 2000
				if(userTime[2] != 0 || userTime[3] != 0){ //if not ##00
					userTime[2] = 0;
					userTime[3] = 0;
					valid = true;
				}
				...
				printf("%d\n", ((int)userTime[0]) >= 0);
				printf("%d\n", ((int)userTime[0]) <= 2);
				printf("User entry: %c, %c, %c, %c\n", userTime[0], userTime[1], userTime[2], userTime[3]);
				printf("Time Error: Please re-enter selection with proper formatting ");
				scanf("%s %s", &dayUser, &userTime);
				valid = false;
			}
			
			if((strcmp(dayUser, dayMWF) > 0) || (strcmp(dayUser, dayTR) > 0)){
				printf("Day Error: Please re-enter selection with proper formatting: ");
				scanf("%s %s", &dayUser, &userTime);
				valid = false;
			}else if(valid == false){
				valid = false;
